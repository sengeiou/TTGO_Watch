...
#define ESP32  //@-20200918 增加这个宏   wifi热点使用wuyiyi，8879网络有限制。
...
//@-固件更新目标文件
String bin = "/media/images/TTGO_2020-Lvgl_Button.ino.twatch.bin"; // bin file name with a slash in front.
...
//@-固件更新任务
void updateFirewarmTask(void *pvParameters) 
{
    vTaskDelay(500);

    Serial.println("Connecting to: " + String(host));

    //@1-Connect to dx1023.com
    if (wifi_update_client.connect(host.c_str(), port)) 
    {
      //@2-获取bin文件
      Serial.println("Fetching Bin: " + String(bin));
  
      //@3-Get the contents of the bin file
      wifi_update_client.print(String("GET ") + bin + " HTTP/1.1\r\n" +
                  "Host: " + host + "\r\n" +
      ...
      while (wifi_update_client.available() == 0) 
      {
      ...
      //@5-获取bin文件
      while (wifi_update_client.available()) 
      {
        //@-读取文件结束标志
        String line = wifi_update_client.readStringUntil('\n');
        //@-去除空格
        line.trim();
    ...
    if (contentLength && isValidContentType) 
    {
      ...
      if (canBegin) 
      {
        ...
        //@-获得接收到bin文件的字节数
        size_t written = Update.writeStream(wifi_update_client);
...
//       // remaining `wifi_update_client` to the
//       // Update.writeStream();
//       if (!line.length()) {
//         //headers ended
//         break; // and get the OTA started
