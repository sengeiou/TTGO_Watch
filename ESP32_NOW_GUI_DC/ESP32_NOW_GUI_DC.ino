...
uint8_t broadcastAddress3[] = {0x84, 0xCC, 0xA8, 0x61, 0x0A, 0x28};    //TTGO_ESP32
...
char  ethNet_RecvBuffer[200];  // buffer to hold incoming packet,
byte  ethNET_SendBuffer[50];        // a string to send back
...
void setup() {
  ...
  memcpy(peerInfo1.peer_addr, broadcastAddress3, 6);
  peerInfo1.channel = 0;  
...
void loop() 
{
  ...
  if(Ethernet_Connect_Flag == true)
  {
    ...
    if(packetSize == 200)
    {
      ...
      Udp_m.read(ethNet_RecvBuffer, 200);
      ...
      // Serial.print("From ");
      // IPAddress remote = Udp_m.remoteIP();
      // for (int i=0; i < 4; i++) {
      ...
      // }
      // Serial.print(", port ");
      // Serial.println(Udp_m.remotePort());

      // read the packet into packetBufffer
      // Udp_xh.read(Snet_ESP32_xh_Recv.DSP_Data_Buff128, 128);

      // Serial.println("Contents:");
      // Serial.println(ethNet_RecvBuffer);

      // myservo.write(ethNet_RecvBuffer[0]);

      // send a reply to the IP address and port that sent us the packet we received
  //    Udp_fy.beginPacket(Udp_fy.remoteIP(), Udp_fy.remotePort());
  //    Udp_fy.write(ethNET_SendBuffer);
  //    Udp_fy.endPacket();
  ...
  if(count > 10)
  {
    ...
    if(Ethernet_Connect_Flag == true)
    {
      ...
      ethNET_SendBuffer[45] = (ESP32_Send_Count & 0xff00);
      ethNET_SendBuffer[46] = (ESP32_Send_Count & 0x00ff);

      Udp_m.beginPacket(ip_m, port_m);
      Udp_m.write(ethNET_SendBuffer,50);  //write byte
      Udp_m.endPacket(); 
  ...
  //   send_Data.e = false;
    ...
    send_Data.FY_Angel_Byte1 = ethNet_RecvBuffer[20];
    send_Data.FY_Angel_Byte2 = ethNet_RecvBuffer[21];
    send_Data.XH_Angel_Byte1 = ethNet_RecvBuffer[22];
    send_Data.XH_Angel_Byte2 = ethNet_RecvBuffer[23];

    send_Data.ZZ_MainPower = ethNet_RecvBuffer[27];
    ...
    result = esp_now_send(broadcastAddress3, (uint8_t *) &send_Data, sizeof(send_Data));
  ...
  if(count_temp > 400)
  {
    ...
    // temp_data1 = ethNet_RecvBuffer[20];
    // temp_data2 = ethNet_RecvBuffer[21];
    // temp_data3 = ((temp_data1 * 256) + temp_data2);
    // if(temp_data1 >= 128)
    // {
    //   temp_data3 = temp_data3 - 65535 -1;
    // }
    // temp_data4 = temp_data3 * 0.02;

    // Serial.print("FY_angel:");
    // Serial.println(temp_data4);

    // temp_data1 = ethNet_RecvBuffer[22];
    // temp_data2 = ethNet_RecvBuffer[23];
    // temp_data3 = ((temp_data1 * 256) + temp_data2);
    // if(temp_data1 >= 128)
    // {
    //   temp_data3 = temp_data3 - 65535 -1;
